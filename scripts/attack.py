#
#    Copyright (C) 2016 Dang Duong
#
#    This file is part of Open Tux World.
#
#    Open Tux World is free software: you can redistribute it and/or modify
#    it under the terms of the GNU General Public License as published by
#    the Free Software Foundation, either version 3 of the License, or
#    (at your option) any later version.
#
#    Open Tux World is distributed in the hope that it will be useful,
#    but WITHOUT ANY WARRANTY; without even the implied warranty of
#    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#    GNU General Public License for more details.
#
#    You should have received a copy of the GNU General Public License
#    along with Open Tux World.  If not, see <http://www.gnu.org/licenses/>.
#
from bge import logic
from random import randint
import math

scene = logic.getCurrentScene()

def shoot_setup(own, parent, scene, name, name_inventory, mass):
    parent["hit_released"] = False
    shoot = scene.addObject(name+"_physics", own, 0)
    shoot.worldScale = [0.197, 0.197, 0.197]
    shoot["owner_ID"] = id(parent)
    shoot.mass = mass
    shoot.setLinearVelocity([-90, 0, 0], True)
    parent[name_inventory] -= 1

def target_aim(cont):
    own = cont.owner
    parent = own.parent
    if own["aim"]:
        target = parent.children["camera_track"].children["camera_track2"].children["cam_dir2"]
        own.alignAxisToVect(own.worldPosition - target.worldPosition, 0, 1.0) # point X axis at target
    else:
        own.alignAxisToVect(parent.worldOrientation.col[0], 0, 1.0) # use parent Y as up axis
    own.alignAxisToVect(parent.worldOrientation.col[1], 1, 1.0) # use parent Y as up axis

def AI_target_aim(cont):
    own = cont.owner
    parent = own.parent
    if not parent["normal"]:
        try:
            target = scene.objects.from_id(parent["attacker_ID"])
            own.alignAxisToVect(own.worldPosition - target.worldPosition,0,1.0) # point X axis at target
            own.alignAxisToVect(parent.worldOrientation.col[1],1,1.0) # use parent Y as up axis
        except:
            parent["normal"] = True

def shoot(cont):
    own = cont.owner
    ray = cont.sensors["shoot_ray"]
    parent = own.parent
    item = parent["item"]
    if own["aim"]:
        if ray.positive and item != 0 and item != 3 and id(ray.hitObject) != id(parent): #alert target
            angle = ray.hitObject.worldOrientation.to_euler().z - own.worldOrientation.to_euler().z
            if math.radians(-180) <= angle < math.radians(-90) or math.radians(180) >= angle > math.radians(90): #check if target looking
                ray.hitObject["normal"] = False
                ray.hitObject["attacker_ID"] = id(parent)
    if parent["hit_released"]: #shoot
        if item == 1:
            shoot_setup(own, parent, scene, "snow_ball", "snow", 1500)
        elif item == 2:
            shoot_setup(own, parent, scene, "ice_cube", "ice", 2000)

def hit(cont):
    own = cont.owner
    parent = own.parent
    if parent["item"] == 0 or parent["item"] == 3:
        hit = scene.addObject("hit", own, 3)
        hit["owner_ID"] = id(parent)
        hit.setLinearVelocity([-45, 0, 0], True)
        parent["hit_released"] = False
